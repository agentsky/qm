...
Keep `secretEnv` as it is: one flat map of values. Add a list of key names per declared service under `services.<name>.secrets`, with defaults that record which process reads which key. The chart renders one `Secret` per Deployment. It holds the union of the host's list and its embedded component's list (auth into portal, admin into web-ui). The Deployment attaches only that Secret. Each service also gets its own `envFrom`, merged into its host for auth and admin. The shared top-level `envFrom` keeps its meaning as the documented escape hatch.
...
The Secret is a named template, so the checksum annotation hashes one workload's Secret instead of the whole file. Rotating a key only the portal carries, such as `AUTH_SIGNING_JWK`, rolls the portal alone.
...
One `helm upgrade`. No values change is needed when every key in `secretEnv` is a secret name the code reads. Every pod rolls once, because its Secret is new, and comes back with a subset of what it had. An operator carrying a non-secret setting or a plugin's secret through `secretEnv` gets a render failure naming each key and moves it to `env` or to that service's list. The probes do not catch a lost key: only core has an HTTP health path, and every process treats a missing key as optional. The render check is the safety net. `helm rollback` restores the single Secret. The chart version goes to 0.3.0.

Verification

CI renders the chart nowhere today. This ships with a `helm lint` and `helm template` check. It asserts one Secret per Deployment with the expected keys, the portal Secret free of the six keys above, one chart-rendered `secretRef` per Deployment and it its own, the alias fixtures with the broker disabled, and an unused key failing with its name.
